...
(async () => {
  ...
  try{
    ...
    await frame.waitForSelector('button[data-name="open-indicators-dialog"]');
    ...
    for (let index = 0; index < indicato.length; index++) {
      ...
      console.log(title)
      await frame.waitForSelector(`div[data-title="${title}"]`);
      await frame.$eval(`div[data-title="${title}"]`, el => el.click());
    }
    await frame.waitForSelector('div[data-name="indicators-dialog"] button[data-name="close"]');
    await frame.$eval('div[data-name="indicators-dialog"] button[data-name="close"]', el => el.click());

    await frame.waitForSelector('button[data-tooltip="Bar\'s style"]');
    await frame.$$eval('button[data-tooltip="Bar\'s style"]', (el) => {el[0].click()});
    await frame.waitForSelector(`div[data-name="menu-inner"] div[data-value="${user_style}"]`);
